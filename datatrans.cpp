...
void parseWithRegex(const std::string& input, int& index, std::string& data, int& retrans) {
  // 定义正则表达式模式
...
void *run_udp_thread(void *obj) {
  ...
  while (1) {
    ...
    if ((len = recvfrom(client->getSockFd(), buffer, 2048, 0,
      ...
      char rcv_buf[2048] = {0};
      ret = ikcp_recv(client->getKcpSocket(), rcv_buf, len);
      if (ret >= 0) // 检测ikcp_recv提取到的数据
      {
        int index = -1;
        std::string data;
        int retrans = 0;
        std::cout << "当前时间" << iclock64() << "收到内容："  << rcv_buf << std::endl;
        ...
        if (retrans == 0) {
          ...
          ss << "index:" << index << "current:" << data << "retrans:" << 1 << std::endl;
